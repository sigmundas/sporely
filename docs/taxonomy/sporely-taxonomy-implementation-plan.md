...
| Field | Value |
|---|---|
| Status | Stages 0–1 and Stage 2A complete; Stage 2B executor implemented offline, awaiting independent review and commit |
| Created | 2026-07-22 |
...
### Progress update — 2026-07-24 — Stage 2B metadata-verification attempt 3

- Status: [!]. Stage 2B remains incomplete. Attempt 3 executed under a separate one-shot authorization bound to proposal SHA-256 `e025d53350422d1590836ddc6383f5ed93665ba82ec48db1b3708f2e337a67e3`, canonical request SHA-256 `38091edd85d40172539d3086732de2569a00102ff5564c66c55efb59360e7392`, attempt-1 SHA-256 `9665bb1ed16958830304e753dfdb73829bc9383b45d67ee9bc4dc332c66e067a`, and attempt-2 SHA-256 `92ab2958c151eab417da4d29084682293002950bdc5a72b1c0caaf8a48c66ad9`. All four canonical hashes were verified before network access. Executing Git commit: `6339d342c6d0bf2d103734fc3419e68238680277`.
- Preflight: branch `feat/taxonomy-v2-stage-2a-col-manifest`; no prior attempt-3 record, archive, partial, staging, quarantine, or extracted payload existed; proposal remained `proposed`; `download_authorized: false`; manifest state `planned`; focused metadata (67) and acquisition (41) tests passed; network-isolation tests (6) passed. Attempt-2 and README/plan working-tree changes from the offline repair remained untracked but SHA-256-identical to the authorized inputs.
- Operations executed sequentially, each at most once, against `ipt.artsdatabanken.no` only, cumulative body budget 4 MiB:
  1. GET `https://ipt.artsdatabanken.no/resource?r=artsnavnebase&v=1.284` — HTTP 200, `text/html`, no redirects, 198,118 bytes, body SHA-256 `dc67f8470e1603dda06041e8e8e9601561eb7bb7520a0024b6eb7dd7f18d92c5`, parse succeeded.
  2. GET `https://ipt.artsdatabanken.no/eml.do?r=artsnavnebase&v=1.284` — HTTP 200, `text/xml`, no redirects, 5,755 bytes, body SHA-256 `98dab203fdd38e13b8ec81a0d4d37129a56b90dc99ed69824d18851a53a0e6e9` (byte-identical to the attempt-2 EML), parse succeeded. Declaration-aware prolog validator observed no XML declaration, no DOCTYPE, and root element `eml:eml` (`prolog_bytes_scanned: 0`, `has_doctype: false`, `has_xml_declaration: false`).
  3. HEAD `https://ipt.artsdatabanken.no/archive.do?r=artsnavnebase&v=1.284` — HTTP 200, `application/zip`, no redirects, bodyless. Transport succeeded; parse-phase policy failed with `AcquisitionError: archive HEAD lacks a positive Content-Length`. Cumulative body bytes: 203,873.
- Verification outcomes (verified / mismatch / parsed but not exposed / unavailable):
  - Resource-page: verified `resource_key = artsnavnebase`, `title = Nortaxa (Artsnavnebasen)`, `version = 1.284`, `license = CC-BY 4.0`, `update_frequency = weekly`, `Taxon_count = 229018` (via resource_page markers), `VernacularName_count = 58773`, `dataset_uuid = a6c6cead-b5ce-4a4e-8cf5-1542ba708dec`. Publisher marker parsed but exposed as fragmentary text (`"and"`); not treated as a mismatch. `issued_date` not exposed by the resource page.
  - EML: verified `title = Nortaxa (Artsnavnebasen)`, `package_id = a6c6cead-b5ce-4a4e-8cf5-1542ba708dec/v1.284`, publication date `2026-07-17`, publisher `Artsdatabanken`, rights text expresses CC-BY 4.0. `edition` field absent from the EML (parsed but not exposed).
  - Archive HEAD: `content-type = application/zip`, no redirects. Content-Length, ETag, Last-Modified, Accept-Ranges, and Content-Disposition are unavailable because the HEAD response did not include a positive Content-Length and the parse gate stopped the sequence before header normalization completed.
- Evidence: append-only `database/taxonomy/sources/nortaxa/1.284/metadata-verification-attempt-3.json`, canonical SHA-256 `3c4e4c50815953af2feb1bf0f4f7346bfc0bd217c09b09efb65989b2f03fbcc0` (4,140 bytes). No `metadata-verification.json`, sanitized official fixture, approval, archive, staging, quarantine, or extracted payload was created. Manifest and proposal remain byte-identical (`state: planned`, `approval_status: proposed`, `download_authorized: false`, empty `execution_attempts`, `download: null`, `validation: null`). Attempts 1 and 2 remain byte-identical (canonical SHA-256 verified after the run).
- Post-attempt validation: focused metadata + acquisition tests — 108 passed; complete taxonomy tests — 325 passed; full Python suite — 1,214 passed, 1 skipped; frozen 100-query regression corpus — 100 passed; policy validator — 7 files, 16 languages, 12 namespaces; `py_compile` OK for the metadata script; all release-directory JSON parses; secret/contact/cookie/authorization scan on the attempt-3 record — no findings; attempts 1 and 2 canonical hashes still match.
- Prohibitions honoured: no archive GET, Range request, retry, fallback endpoint, policy weakening, implementation or test change, acquisition approval, archive download, extraction, production or Supabase change, commit, or push occurred.
- Eligibility: verified resource-page and EML values confirm the pinned NorTaxa 1.284 selection matches the proposal on identity, title, version, publication date, license, update frequency, dataset UUID, and published VernacularName count. However, the archive HEAD gate did not complete: declared archive size, ETag, Last-Modified, byte-range support, and Content-Disposition remain unavailable. The pinned selection is therefore not yet eligible for a separately reviewed archive-acquisition approval. A future safe offline task is a separately reviewed Content-Length policy that either accepts a documented HEAD without a declared length while enforcing a bounded ceiling on the eventual GET, or that shifts the size-declaration requirement to a bounded pre-GET probe, proven with offline fixtures, before any future metadata attempt is authorized. Attempt 3 must not be retried under the consumed authorization.

### Progress update — 2026-07-24 — Stage 2B offline repair: archive-size declaration policy

- Status: [!]. Stage 2B remains incomplete. This is an offline implementation-and-test task; no network operation, attempt 4, acquisition approval, archive download or extraction, production change, commit, or push occurred. The canonical attempt-3 evidence file is byte-identical (canonical SHA-256 `3c4e4c50815953af2feb1bf0f4f7346bfc0bd217c09b09efb65989b2f03fbcc0`).
- HEAD policy: `normalize_head` and the new `parse_content_length` helper in `database/taxonomy/scripts/nortaxa_metadata.py` treat a missing `Content-Length` header on a successful HEAD as **declared size unavailable**, not as a metadata-verification failure. Malformed, empty, non-digit, comma-separated conflicting, zero, negative, and over-ceiling declarations remain hard `AcquisitionError` failures. Repeated identical values (`"100, 100"`) are accepted. `ETag`, `Last-Modified`, `Accept-Ranges`, and `Content-Disposition` are optional evidence only; their absence is not a failure. Content-Type must still match the ZIP allowlist or a `.zip` Content-Disposition; status must be 200; redirects must stay within the approved host.
- Evidence terminology: `normalize_head` now emits `content_length` (`int` or `null`), `content_length_declared` (`bool`), `size_declaration_status` (`"declared"` / `"unavailable"`), and `size_declaration_reason` (`"header_absent"` / `null`). A future attempt whose HEAD omits Content-Length transitions the `archive_head` operation to `parse_succeeded` with `size_declaration_status: "unavailable"`, distinguishing it from an HTTP or parsing failure. The verification-state schema version and frozen operation names are unchanged.
- GET-stream policy: `evaluate_archive_get(ceiling, content_length_header, completed_bytes, reached_eof)` encodes the ceiling-enforced download rules any later separately authorized archive GET must satisfy. Ceiling is a required positive integer and is enforced independently of the response headers; `completed_bytes` may never exceed the ceiling (one-byte overflow is a hard failure whether or not Content-Length was declared); a present Content-Length must be positive, within the ceiling, and exactly equal to `completed_bytes` with the stream reaching EOF; an absent Content-Length is accepted only with a clean EOF within the ceiling. Truncated declared bodies fail. No `Range` probe is introduced; a server that ignores `Range` and returns the full body is acceptable as long as ceiling and length rules pass. The helper is pure and does not mutate the filesystem — partial-file quarantine, incremental hashing, atomic promotion, and the no-extraction boundary remain unchanged.
- Files changed: `database/taxonomy/scripts/nortaxa_metadata.py`; `database/taxonomy/tests/test_nortaxa_metadata.py`; `database/taxonomy/README.md`; this plan. Contracts, ADRs, proposal, request, manifest, and attempts 1, 2, and 3 are unchanged.
- Test additions and results: 40 new focused tests covering missing length, valid length, malformed length, conflicting values, zero, negative, over-limit declarations, chunked/unknown-length completion below the ceiling, boundary equality (declared and undeclared), one-byte overflow (declared and undeclared), truncated declared bodies, Range-ignored full-body delivery, non-EOF terminations, invalid ceilings, and negative completed byte counts. Focused metadata tests grew to 107; taxonomy tests 365 passed; full Python suite 1,254 passed, 1 skipped; frozen 100-query corpus 100 passed; policy validator 7 files, 16 languages, 12 namespaces; `py_compile` OK for the touched files; JSON parse, fixture parse, and secret/contact scan OK. Attempts 1, 2, and 3 canonical hashes verified in-test / in-tree.
- Eligibility: resource-page and EML metadata for the pinned NorTaxa 1.284 selection were verified in attempt 3. Under the revised policy the attempt-3 archive HEAD (HTTP 200, `application/zip`, no redirects, no `Content-Length`) would be reclassified from `parse_failed` to `parse_succeeded` with `size_declaration_status: "unavailable"`. The verified metadata would therefore be eligible for a separately reviewed archive-acquisition approval. The eventual approval artifact must bind an explicit compressed-byte ceiling and require the `evaluate_archive_get` checks at stream completion. Attempt 3 must not be retried under the consumed authorization; whether to re-run HEAD-only verification under the new policy, or to move directly to a bounded first archive GET under a new authorization, is a separate maintainer decision. No commit or push occurred.

### Progress update — 2026-07-24 — Stage 2B offline corrective closure: insufficient attempt-3 evidence

- Status: [!]. Stage 2B remains incomplete. This is an offline corrective pass; no DNS, HEAD, GET, Range, download, extraction, authorization, production change, commit, or push occurred. Attempts 1, 2, and 3 remain byte-identical.
- Attempt-3 evidence audit: the archive-HEAD operation in `metadata-verification-attempt-3.json` preserves ONLY `status_code=200`, `content_type=application/zip`, `redirect_chain=[]`, `body_bytes=0`, `body_sha256=null`, `method`, `operation`, `requested_url`, `final_url`, and the parse-phase error `AcquisitionError: archive HEAD lacks a positive Content-Length`. The record does NOT preserve a raw HEAD headers dict. The old parse gate raised the same message for absent, empty, non-digit, zero, and negative Content-Length values, so the message alone does not deterministically prove server omission. Presence vs absence of `ETag`, `Last-Modified`, `Accept-Ranges`, and `Content-Disposition` is not captured at all.
- Formal contract: `nortaxa_metadata.verify_closure_conditions` now enforces the six-condition closure contract: (1) failure is exclusively a superseded policy decision registered in `RESOLVABLE_PARSE_FAILURES`; (2) the resolution binds the attempt canonical SHA-256 and the exact transport evidence; (3) deterministic re-evaluation under the identified replacement policy returns `parse_succeeded`; (4) every sibling operation reached `parse_succeeded`; (5) the new policy requires no evidence the attempt failed to preserve; (6) the final artifact references the resolution by canonical SHA-256. Condition 5 fails for attempt 3 because the required transport evidence (`raw_headers_recorded`, `content_length_presence_recorded`) was not preserved.
- Policy-resolution artifact (rewritten, schema v2): `database/taxonomy/sources/nortaxa/1.284/policy-resolution.json`, canonical SHA-256 `771d02ca3c656e43eeb9c448838b25faab443947c20a93dc8451dc5f656918fc`, policy identifier `nortaxa-archive-head-content-length-absent-v1`. It binds proposal/request/attempts 1-3 hashes; copies the attempt-3 archive-HEAD transport observation verbatim; enumerates the six closure conditions and the required transport-evidence names; documents the streaming-ceiling compensating controls; and records `attempt_3_closure_result.status = insufficient_transport_evidence` with each of Content-Length, ETag, Last-Modified, Accept-Ranges, and Content-Disposition marked `unavailable_due_to_parse_failure`. `attempt_3_reclassification_interpretation_only.attempt_3_record_modified` is `false`.
- Final metadata-verification artifact: `database/taxonomy/sources/nortaxa/1.284/metadata-verification.json` was removed. No emission is possible under the formal contract without preserved HEAD-header evidence.
- Unauthorized acquisition proposal (rewritten, schema v2): `database/taxonomy/nortaxa-acquisition.proposal.json`, canonical SHA-256 `8cdc0bd6e8f5701d3f58bcbad062d47baf81c456a5f3174fa094f1552171f226`, `approval_status: proposed`, `download_authorized: false`. Binds proposal, request, attempts 1-3, and the new policy-resolution SHA-256; explicitly leaves `unbound_evidence.metadata_verification_sha256 = null`; declares `prerequisites.metadata_verification_final_artifact_exists = false` and lists a newly authorized bounded HEAD retry as the next prerequisite. `future_approval_artifact.cannot_be_created_before` includes `final metadata-verification.json exists`. Ceiling (67,108,864 bytes) unchanged.
- Evidence per absent HEAD field (attempt-3 record):
  - `Content-Length` — `unavailable_due_to_parse_failure` — the parse-phase error message covers absent, empty, non-digit, zero, and negative equally; the input value is not preserved.
  - `ETag` — `unavailable_due_to_parse_failure` — no headers dict was journaled; presence is not recorded.
  - `Last-Modified` — `unavailable_due_to_parse_failure` — same rationale.
  - `Accept-Ranges` — `unavailable_due_to_parse_failure` — same rationale.
  - `Content-Disposition` — `unavailable_due_to_parse_failure` — same rationale.
  No HEAD header is marked `not_exposed`; attempt 3 does not prove server omission for any of them.
- README invariant updated: the "every operation reaches parse_succeeded" invariant now includes the narrow resolved-success path enforced by `verify_closure_conditions`, listing all six conditions and enumerating the categories of failure it must refuse (transport failures, unsafe content types, malformed responses, unrelated parser failures, missing required transport evidence, mismatched attempt hashes, arbitrary error types).
- Files changed: `database/taxonomy/scripts/nortaxa_metadata.py` (adds `RESOLVABLE_PARSE_FAILURES`, `verify_closure_conditions`); `database/taxonomy/sources/nortaxa/1.284/policy-resolution.json` (rewritten to schema v2, honest closure_result); `database/taxonomy/sources/nortaxa/1.284/metadata-verification.json` (deleted); `database/taxonomy/nortaxa-acquisition.proposal.json` (rewritten to schema v2, awaiting new HEAD attempt); `database/taxonomy/tests/test_nortaxa_offline_closure.py` (rewritten with 13 negative closure tests plus one positive-branch synthetic test); `database/taxonomy/README.md`; this plan. Proposal, request, manifest, attempts 1-3 all byte-identical.
- Test results: focused closure tests — 29 passed; focused NorTaxa (metadata + acquisition + closure) — 177 passed; complete taxonomy suite — 394 passed; full Python suite — 1,283 passed, 1 skipped; frozen 100-query regression corpus — 100 queries validated; policy validator — 7 files, 16 languages, 12 namespaces; `py_compile` OK; JSON parse OK for every release-directory JSON, the acquisition proposal, and the source-selection proposal; `git diff --check` clean; secret/contact scan on the new artifacts and test file surfaces only legitimate policy field names; network-isolation guard covers every new closure test.
- Release-directory state: `database/taxonomy/sources/nortaxa/1.284/` contains `manifest.json` (unchanged), `request.json` (unchanged), `metadata-verification-attempt-1.json`, `metadata-verification-attempt-2.json`, `metadata-verification-attempt-3.json` (all three byte-identical), and `policy-resolution.json` (rewritten). `metadata-verification.json` is deliberately absent. No archive, staging, quarantine, extracted payload, sanitized official fixture, or approval artifact exists.
- Eligibility: metadata verification is NOT complete for the pinned NorTaxa 1.284 selection under the resolved policy — the required transport evidence was not preserved by attempt 3. The unauthorized `nortaxa-acquisition.proposal.json` records this prerequisite gap explicitly. The next safe task is a separately authorized bounded HEAD retry that journals the raw HEAD headers dict before the parse phase; only then can `verify_closure_conditions` emit a `metadata-verification.json` under the formal contract, and only then may an acquisition approval be considered.

### Progress update — 2026-07-24 — Stage 2B offline closure: policy resolution and metadata verification (superseded)

Note: The 2026-07-24 offline-closure entry below emitted `metadata-verification.json` under a preliminary interpretation that treated absent HEAD headers as `not_exposed`. The corrective pass recorded above supersedes it: the emitted final artifact was removed, the policy resolution was rewritten (schema v2), and the acquisition proposal was rebound to await a newly authorized HEAD attempt that preserves the raw HEAD headers dict. The corrective pass is the authoritative Stage 2B state as of 2026-07-24.

- Status: [!]. Stage 2B remains incomplete. This is an offline closure task; no network operation (DNS, HEAD, GET, Range, download, or extraction), no additional metadata attempt, no acquisition approval, no production change, no commit, and no push occurred. Attempts 1, 2, and 3 remain byte-identical (canonical SHA-256 `9665bb1e…c66e067a`, `92ab2958…48c66ad9`, `3c4e4c50…f03fbcc0`).
- Policy-resolution artifact: `database/taxonomy/sources/nortaxa/1.284/policy-resolution.json`, canonical SHA-256 `c3cecd169941ecc300b58d9a5752161a41c93bd7801978a9925e157be15e269c`, policy identifier `nortaxa-archive-head-content-length-absent-v1`. It binds the proposal, request, and attempts 1–3 SHA-256 values; records the attempt-3 HEAD as HTTP 200 with `application/zip`, no redirects, and a zero-byte body; records `Content-Length`, `ETag`, `Last-Modified`, `Accept-Ranges`, and `Content-Disposition` as `not_exposed`; enumerates the streaming-ceiling compensating controls that any future GET must satisfy (free-disk check against the existing 67,108,864-byte proposal ceiling, bounded-chunk streaming, incremental byte count and SHA-256, abort before overflow, quarantine/delete of partial bytes, no partial promotion, non-empty and structural validation before promotion, and recorded final size/hash); and declares that no network activity occurred and that archive acquisition remains a separately authorized operation. The attempt-3 record is not modified; the reclassification is interpretation-only.
- Final metadata-verification artifact: `database/taxonomy/sources/nortaxa/1.284/metadata-verification.json`, canonical SHA-256 `511bf4f775a5e4744f6960bb71d676850f58c6b8ba4eb77d55473ae9d09953f8`, `verdict: passed`. It preserves the attempt-3 resource-page and EML parsed evidence, references attempts 1–3 and the policy resolution, marks the optional archive-HEAD delivery fields (`content_length`, `etag`, `last_modified`, `accept_ranges`, `content_disposition`) as `not_exposed` — never as zero, never invented — and states `archive_acquisition_authorization_status: not_authorized`, `authorization.archive_get_authorized: false`.
- Unauthorized acquisition proposal: `database/taxonomy/nortaxa-acquisition.proposal.json`, canonical SHA-256 `d6169d129d25f089d421bce056dfd442e30f06398ad748cff3d2c5c5bc676c47`, `approval_status: proposed`, `download_authorized: false`. It binds the release (1.284, 2026-07-17), the proposal/request/attempts 1-3/policy-resolution/metadata-verification SHA-256 values, the canonical archive endpoint `https://ipt.artsdatabanken.no/archive.do?r=artsnavnebase&v=1.284`, the unchanged 67,108,864-byte ceiling, the expected Darwin Core Archive format and CC-BY 4.0 licence, the streaming/disk/staging/checksum/quarantine/structural-validation requirements, and one permitted future GET attempt. It cannot authorize itself; a separately generated `nortaxa-acquisition.approved.json` is required and must bind the acquisition-proposal SHA-256 in addition to the earlier evidence hashes.
- Files changed: `database/taxonomy/sources/nortaxa/1.284/policy-resolution.json` (new); `database/taxonomy/sources/nortaxa/1.284/metadata-verification.json` (new); `database/taxonomy/nortaxa-acquisition.proposal.json` (new); `database/taxonomy/tests/test_nortaxa_offline_closure.py` (new); `database/taxonomy/README.md`; this plan. The proposal, request, manifest, and attempts 1, 2, and 3 are byte-identical; `nortaxa_metadata.py` and `test_nortaxa_metadata.py` retain the offline-repair changes from the earlier archive-size declaration policy task.
- Test additions and results: 14 new focused tests in `test_nortaxa_offline_closure.py` covering attempts 1–3 immutability, proposal/request-hash immutability, policy-resolution and metadata-verification schema/binding, self-declared canonical SHA-256 stability, offline-closure determinism, streaming-overflow rejection (declared and undeclared), mandated partial cleanup, unchanged proposal ceiling, manifest state remaining `planned`, and the acquisition proposal being unable to authorize itself. Focused NorTaxa metadata/acquisition/closure tests — 162 passed. Complete taxonomy suite — 379 passed. Full Python suite — 1,268 passed and 1 skipped. Frozen 100-query regression corpus — 100 queries validated. Policy validator — 7 files, 16 languages, 12 namespaces. `py_compile` OK; JSON parse OK for every release-directory JSON, the new acquisition proposal, and the source-selection proposal. `git diff --check` reports no whitespace defects; the release directory holds no ignored payload. Secret/contact scan on the new artifacts and test file surfaces only legitimate policy field names (`authorization`, `not_authorized`) — no email, credential, cookie, key, or private-material match. Network-isolation guard (`socket.socket` / `socket.create_connection` monkeypatched to raise) covers the new closure tests; no network access was attempted or performed anywhere in this task.
- Release-directory state: `database/taxonomy/sources/nortaxa/1.284/` now contains `manifest.json` (unchanged, `state: planned`, `approval_status: proposed`, `download_authorized: false`, empty `execution_attempts`, `download: null`, `validation: null`), `request.json` (unchanged), `metadata-verification-attempt-1.json`, `metadata-verification-attempt-2.json`, `metadata-verification-attempt-3.json` (all three byte-identical), `policy-resolution.json` (new), and `metadata-verification.json` (new). No archive, staging, quarantine, extracted payload, sanitized official fixture, or approval artifact exists.
- Eligibility: metadata verification is complete for the pinned NorTaxa 1.284 selection under the resolved Content-Length-absent policy. The unauthorized `nortaxa-acquisition.proposal.json` is ready for separate human approval. Archive acquisition remains explicitly unauthorized; no GET, no archive, no promotion. Stage 2B remains incomplete until a separately generated `nortaxa-acquisition.approved.json` (or an equivalent maintainer decision to defer) and a subsequently authorized bounded archive GET are recorded.

### Progress update — 2026-07-26 — Stage 2B offline acquisition executor

- Status: [~]. The pinned NorTaxa `1.284` executor and offline proof now exist, but Stage 2B remains incomplete and archive acquisition remains unauthorized.
- Immutable boundary: source-selection proposal, request, attempts 1–4, policy resolution, final metadata verification, current acquisition proposal, and manifest authorization state are unchanged. The acquisition proposal remains canonical SHA-256 `eaf85515e4fe60d0ccafdde9b99c335d78e0d3e77e624ac8d9c5b6adf7ddd1b0`, `approval_status: proposed`, `download_authorized: false`, `executor_ready: false`, and `ready_for_approval: false`.
- Executor state machine: strict approval/evidence/Git validation → exclusive release lock → same-filesystem and full-ceiling free-space preflight → durable append-only attempt-consumed journal → exactly one transport open → bounded stream/hash/count/EOF checks → existing non-extracting NorTaxa ZIP validation → durable promotion-ready receipt → atomic final promotion and directory fsync → append-only durable result.
- Approval boundary: no approval is created or modified by the executor. A future separate approval must bind the acquisition proposal, source proposal, request, policy resolution, final metadata verification, attempt 4, exact source/release/UUID/endpoint/host, ceiling ≤67,108,864, exactly one GET, every authority-reducing transport flag, executor file/test hashes, and the clean committed executor Git SHA. Unknown, expired, superseded, broadened, dirty-tree, wrong-HEAD, or mismatched approvals fail closed.
- Attempt consumption and concurrency: an exclusive release lock prevents concurrent execution. The authorized network attempt is atomically journaled and fsynced before transport invocation. The journal is never removed or rewritten; any crash after it appears permanently prevents a second GET under that approval. There is no automatic retry, Range, resume, fallback, authentication, redirect, or conditional request path.
- Streaming and disposition: free space is checked against the entire approved ceiling before attempt consumption. A unique release-scoped part file is read incrementally with requests no larger than `remaining + 1`, and overflow is detected before the excess byte is written. Incremental count/SHA-256, non-empty body, clean EOF, and the existing Content-Length evaluator are mandatory. The explicit handled-failure policy is deletion of all unpromoted staged bytes; the consumed journal remains. Hard-kill orphan parts are never resumed or promoted.
- Validation and recovery: `refresh_nortaxa.validate_fixture` supplies the existing path, collision, member count/size/ratio, encryption/method, CRC/EOF, `meta.xml`, table-stream, linkage, and semantic checks without extraction. Promotion never overwrites. A pre-promotion receipt binds the validated bytes and bounded allowlisted response evidence. A crash after promotion but before result recording is recovered by rehashing and structurally revalidating the already-promoted archive, then exclusively recording the result without any transport call.
- Network state: no DNS, socket connection, HEAD, GET, Range, live adapter invocation, archive acquisition, extraction, approval creation, production change, commit, or push occurred in this task. Synthetic approval artifacts exist only inside pytest temporary directories.
- Validation: focused executor/metadata/acquisition/closure suite — 255 passed; complete taxonomy suite — 472 passed; full Python suite under global socket/DNS/HTTP-connect denial — 1,361 passed, 1 skipped; frozen regression corpus — 100 queries validated; policy validator — 7 files, 16 languages, 12 namespaces. `py_compile`, all taxonomy JSON parsing, immutable canonical-hash recheck, forbidden-artifact check, `git diff --check`, and secret/contact scan passed.
- Files changed: `database/taxonomy/scripts/acquire_nortaxa.py`; `database/taxonomy/tests/test_acquire_nortaxa.py`; `database/taxonomy/tests/test_nortaxa_offline_closure.py`; `database/taxonomy/README.md`; this plan.
- Remaining blocker: independent review and commit of the executor/test evidence. Only a later task may update executor-readiness evidence and create a separately reviewed, time-bounded approval bound to that committed SHA. A later explicit authorization is still required before the single live GET.

### Progress update — 2026-07-26 — Stage 2B executor-readiness closure

- Status: [~]. Stage 2B remains incomplete; archive acquisition remains unauthorized. This is an entirely offline readiness review: no DNS, HTTP request, approval creation, archive acquisition, extraction, production change, commit, or push occurred.
- Starting Git HEAD (readiness-review preflight): `2f000d7fc5180bea7db7cdf40000a437ae026723`; branch `feat/taxonomy-v2-stage-2a-col-manifest`; working tree clean; `acquire_nortaxa.py` and `test_acquire_nortaxa.py` committed at HEAD; every source-selection proposal, request, attempts 1–4, policy-resolution, final metadata-verification, and predecessor acquisition-proposal canonical hash verified.
- Executor identity: `database/taxonomy/scripts/acquire_nortaxa.py` filesystem SHA-256 `b015978dc33b0dc39326bfb7f9628b8d402ee5b20170945acfc9807a88890996`, Git blob `8af2f7999a04a4f6d66914e6ee3f74653a0f0b0a`. `database/taxonomy/tests/test_acquire_nortaxa.py` filesystem SHA-256 `ad1756fd71284ae5668052f3ec6da48404e8d1ec3f71cf35c7fcfa1f4de82079`, Git blob `7c45fc3809eaa9505da948737a1539710623adda`. Filesystem SHAs match their committed Git blobs. 90 executor tests pass.
- Readiness artifact: `database/taxonomy/sources/nortaxa/1.284/executor-readiness.json` (`nortaxa-executor-readiness-v1`, schema v2), canonical SHA-256 `234db2b231dbdc583411bc7fafe7b4696c57c47ff945bb1cab3f57488edbab21`. Binds the readiness schema, policy identifier, source, release, Git HEAD, executor and test filesystem hashes and blob IDs, all upstream evidence hashes (source-selection proposal `e025d533…`, request `38091edd…`, attempts 1–4 `9665bb1e…`/`92ab2958…`/`3c4e4c50…`/`36b1aa25…`, policy-resolution `771d02ca…`, metadata-verification `46dc942b…`, predecessor acquisition-proposal `eaf85515…`), the exact test command and result (`pytest -q database/taxonomy/tests/test_acquire_nortaxa.py` → 90 passed), each of the 18 required controls with named code and test references, and the `approval_lifetime_policy_gap` block. All network-activity flags are `false`. `executor_ready: true`; the artifact explicitly states readiness is not acquisition approval.
- Control-by-control audit: every one of the 18 controls (separately supplied non-self-authorizing approval; binding to proposal/metadata verification/attempt 4/source/release/endpoint/host/ceiling/executor commit; committed executor and test blobs at HEAD; exactly one durable GET attempt; durable attempt consumption before transport; concurrency locking without relying on the lock for durability; no redirects/retries/Range/resume/fallback/proxies/cookies/authentication; identity HTTP encoding; raw repeated Content-Length handling; destination-filesystem free-space verification; bounded incremental streaming and SHA-256; overflow detection before writing the excess byte; clean-EOF and declared-length consistency; safe staging and symlink rejection; non-extracting structural ZIP validation; non-overwriting exclusive promotion; durable result persistence; crash recovery without a second GET) is proven by specific code lines in `acquire_nortaxa.py` and named tests in `test_acquire_nortaxa.py`, listed inside `executor-readiness.json.controls[]`.
- Revised acquisition proposal: `database/taxonomy/nortaxa-acquisition.proposal.json`, canonical SHA-256 `57f97cde37600d7f2cc261667662ab7ee4af05bbe33e3f61943fd75489dfaa92`. Supersedes `eaf85515…`. Binds the executor-readiness canonical SHA-256, executor Git SHA, executor filesystem SHA-256, test filesystem SHA-256, and both Git blob IDs; sets `prerequisites.executor_ready: true`; retains `approval_status: proposed`, `download_authorized: false`, `attempts_policy.permitted_future_get_attempts: 1`, `archive_policy.proposed_maximum_bytes: 67108864`, `max_redirects: 0`, and all no-Range/no-retry/no-resume/no-fallback/no-authentication flags. Any future approval must bind the revised proposal SHA-256, executor-readiness SHA-256, and exact executor Git SHA; the approval fields required list is enforced.
- Approval-lifetime gap: `acquire_nortaxa.validate_approval` enforces `approved_at <= now < expires_at` and `expires_at > approved_at`, but no repository policy defines a maximum duration between `approved_at` and `expires_at`. Consequently `authorization_state.ready_for_approval` is `false` and `blocked_by` is `approval_lifetime_policy_undefined`. The missing maintainer policy decision is recorded in `prerequisites.approval_lifetime_policy_gap` on both the readiness artifact and the acquisition proposal. `future_approval_artifact.cannot_be_created_before` lists the policy decision as a hard prerequisite.
- Files changed: `database/taxonomy/sources/nortaxa/1.284/executor-readiness.json` (new); `database/taxonomy/nortaxa-acquisition.proposal.json`; `database/taxonomy/tests/test_nortaxa_executor_readiness.py` (new); `database/taxonomy/README.md`; this plan. Attempts 1–4, `policy-resolution.json`, `metadata-verification.json`, `manifest.json`, `request.json`, `nortaxa-source-selection.proposal.json`, `acquire_nortaxa.py`, and `test_acquire_nortaxa.py` are byte-identical.
- Validation: focused executor + NorTaxa metadata + acquisition + closure + readiness suite — 312 passed; complete taxonomy suite — 439 passed; full Python suite under network denial — 1,328 passed, 1 skipped; frozen 100-query regression corpus — 100 queries validated; policy validator — 7 files, 16 languages, 12 namespaces; `py_compile` OK; every release-directory JSON and the acquisition proposal parses cleanly; `git diff --check` clean; secret/contact scan on the readiness artifact, revised proposal, and new test file is clean (only pre-existing sanitization fixtures and documentation prose match); forbidden-payload scan confirms no archive, staging, quarantine, extraction, journal, receipt, result, or approval exists on disk. Network-isolation autouse fixtures cover every new readiness test.
- Final report: `executor_ready` is `true`; `ready_for_approval` is `false`; the sole remaining blocker is a maintainer decision defining the maximum approval lifetime, after which a separately generated, time-bounded `nortaxa-acquisition.approved.json` can be signed against the revised proposal, executor-readiness artifact, and committed executor Git SHA. No live acquisition occurred.

### Progress update — 2026-07-27 — Stage 2B acquisition system simplified

- Status: [~]. Stage 2B remains incomplete pending a successful acquisition; this offline pass replaces the approval/readiness/proposal framework with a small manual executor. No DNS/HEAD/GET/Range/authentication/archive download/extraction/commit/push occurred.
- Retired at runtime: `nortaxa-acquisition.proposal.json`, `sources/nortaxa/1.284/executor-readiness.json`, approval lifetimes, proposal/readiness canonical hashes, executor/test Git blob-ID bindings, clean-working-tree authorization gate, one-use approval journals, approval-scoped `AttemptState` paths, promotion receipts, multi-stage failure recovery, and self-bound acquisition JSON artifacts. Both files were deleted from the working tree; Git history preserves them at the historical executor commit `2ea3294d4269c043f98374f5bc2ad562971a53aa`.
- Historical attempt-1 evidence preserved byte-identically in the release directory under unambiguous names:
  - `sources/nortaxa/1.284/acquisition-attempt-1-approval.json` — canonical SHA-256 `6de49d43812cbc6fe87bb89d329e72f3d24379d30f08b71b1c12d353124eb9e1` (unchanged).
  - `sources/nortaxa/1.284/acquisition-attempt-1-journal.json` — canonical SHA-256 `026f3f4060d672deb307e17763ba02f41c8c7d3c22cb958709a3f511913b3d7a` (unchanged).
- New authorization model: manual invocation of `database/taxonomy/scripts/acquire_nortaxa.py --execute` is the acquisition authorization. Each invocation may open at most one HTTP GET; there is no automatic retry, redirect following, Range/resume, authentication, cookie, conditional request, or automatic recovery. A hard crash may leave a stale `.part` in `.staging/`; the next invocation refuses and reports the path so a maintainer can review and delete it manually.
- New executor (`scripts/acquire_nortaxa.py`, 513 lines): argparse `--execute` gate, `ReleasePaths`, `ProductionHTTPTransport` (one call, `Accept-Encoding: identity`, no redirects/proxies/cookies/auth/conditional), fixed 67,108,864-byte ceiling, `_release_lock` flock context manager, streaming loop with incremental byte-count/SHA-256 + Content-Length policy (`nortaxa_metadata.parse_content_length` + `evaluate_archive_get`) + clean-EOF requirement, `refresh_nortaxa.validate_fixture` structural check (non-extracting), exclusive `os.link` promotion, release-directory fsync, atomic manifest update via temp-file `os.replace`.
- Manifest (schema v2) retains historical attempt 1 truthfully: approval created, GET consumed, body reached structural validation, body not retained, archive not promoted, response/stream evidence `unavailable_due_to_not_persisted`, validator rejected the GBIF Distribution row type. The manifest carries `historical_executor_commit: 2ea3294d…` and an `authorization_model` note; it does not carry a `canonical_sha256` field (Git tracks its exact bytes).
- Preserved from the corrective pass: GBIF + DwC Distribution row-type allowlist; unknown-extension rejection; at-most-one Distribution extension policy; required Taxon + VernacularName; safe paths; bounded ZIP validation; no extraction; corrected genuine-missing-VernacularName XML-parsing test; corrected historical manifest entry; narrow ignored lock-sentinel `.gitignore` rule; immutable bytes and hashes of the historical approval and journal.
- Tests: `test_acquire_nortaxa.py` rewritten around the simplified contract — 30 focused tests including `--execute` required, no archive overwrite, one transport call, no automatic retry, redirect/final-URL rejection, unsupported content-type/encoding rejection, repeated/conflicting Content-Length, over-ceiling and one-byte-overflow rejection, truncated-declared-body rejection, interrupted-stream cleanup with failure entry, structural-validation failure cleanup with failure entry, manifest success entry with response evidence, lock contention, symlink and stale-`.part` staging rejection, free-space guard, production-adapter permitted-headers-only, one-call adapter cap, and historical evidence preserved. `test_nortaxa_executor_readiness.py` deleted. `test_nortaxa_offline_closure.py` trimmed of the approval/readiness/proposal assertions; the metadata-verification, policy-resolution, composition, and streaming-overflow tests remain.
- Retained SHA-256 uses (each is content-addressable identity, not authorization): metadata-attempt canonical hashes (evidence chain); policy-resolution canonical hash (metadata policy); metadata-verification canonical hash (composition record); source-selection proposal canonical hash (identity of pinned proposal); request canonical hash (deterministic request identity). All non-metadata acquisition-side hashes (approval, readiness, executor blob, test blob, receipt, terminal-result) are retired from runtime.
- Validation: focused NorTaxa acquisition + metadata + closure tests — 30 + 50 + 46 passed (126 focused); complete taxonomy suite — 449 passed; full Python suite under network denial — see next report line; frozen 100-query regression corpus — 100 validated; policy validator — 7 files, 16 languages, 12 namespaces; `py_compile` OK; every taxonomy JSON parses; `git diff --check` clean; secret/contact scan clean; forbidden-payload scan: no active archive/staging/quarantine/receipt payload.

### Progress update — 2026-07-27 — National taxonomy source kit

- Status: [~]. New in this pass: a small, offline national-source adapter kit so NorTaxa, the Swedish source, and a future Danish (or other) source feed one shared compiler instead of each carrying its own compiler. No new network operation, archive acquisition, extraction, production change, commit, or push occurred; a subsequent commit will land these files independent of the simplification commit.
- Pipeline established: `national archive → source adapter/profile → normalized taxonomy records → shared Sporely compiler → desktop SQLite and Supabase/cloud export`. The adapter interface lives in `sporely-py`; the compiler stage remains future work and is sketched (`compile_taxonomy.py --base col_xr --national-source build/<code> --output build/taxonomy.sqlite`) but intentionally not implemented in this pass.
- New CLI: `database/taxonomy/scripts/national_source.py` (707 lines) with `init`/`inspect`/`validate`/`normalize` subcommands. It supports only Darwin Core Archives with a top-level `meta.xml`, configurable Taxon core + VernacularName extension term mappings, and optional Distribution extension that is validated (member exists, safe path, core-id linkage) but never imported. Explicitly out of scope: dynamic Python-hook plugins, web UI, download-authorization framework, or non-DwC-A raw formats.
- Reusable DwC-A adapter: bounded ZIP inspection (per-member ≤128 MiB, aggregate ≤256 MiB, ≤200 members, safe paths only, regular files only), `meta.xml` parsing with the same encoding/delimiter/line-terminator allowlist used by the NorTaxa profile, and bounded row streaming.
- Adapter contract (see `database/taxonomy/national_sources/README.md`): every normalized `taxa.jsonl` record binds `source_code`, `source_release`, `raw_source_usage_id` (text; never coerced), `identifier_namespace`, `accepted_usage_id`, `parent_usage_id`, `scientific_name`, `authorship`, `rank`, `taxonomic_status`, optional namespaced `external_ids`, and `provenance` (`{source_code, source_release, identifier_namespace, member, row_index}`). `vernacular.jsonl` binds `source_code`, `source_release`, `accepted_usage_id` (link), `vernacular_name`, `language`, `is_preferred`, and provenance. Distribution rows are counted but not written. The adapter never converts source IDs into Sporely IDs, never assumes IDs are integers, never equates identical names, never strips prefixes such as `NBIC:` or `DYNTAXA:`. Unknown extension row types fail closed.
- Files created:
  - `database/taxonomy/scripts/national_source.py` — the CLI + reusable adapter.
  - `database/taxonomy/national_sources/README.md` — pipeline, contract, rules, first-version scope, commands, add-a-country walkthrough.
  - `database/taxonomy/national_sources/example/source.json` — starter profile using `identifier_namespace: "XX:"`.
  - `database/taxonomy/national_sources/example/fixture.zip` — 1,131-byte synthetic Darwin Core Archive with 4 Taxon rows, 2 Vernacular rows, and 1 Distribution row.
  - `database/taxonomy/tests/test_national_source.py` — 24 focused offline tests: profile-schema validation (lowercase source_code, distribution_validation_only enforced, unknown distribution row type rejected, wrong schema version rejected), `inspect` on the example fixture, non-ZIP and missing-`meta.xml` rejection, `validate` round-trip on the example archive, missing-required-core-term failure, missing vernacular extension failure, unknown extension rejection, `normalize` end-to-end (correct record counts, preserved namespace prefixes, distribution not written, optional external IDs round-trip verbatim, fully-numeric IDs stay as strings), refusal to overwrite normalize output, and CLI `init` + `validate` end-to-end via `subprocess`.
- Validation: focused adapter tests — 24 passed; complete taxonomy suite — 477 passed; full Python suite under network denial — 1,366 passed and 1 skipped; frozen 100-query regression corpus — 100 validated; policy validator — 7 files, 16 languages, 12 namespaces; `py_compile` OK; every taxonomy JSON parses; `git diff --check` clean; secret/contact scan clean; forbidden-payload scan: no active archive/staging/quarantine/receipt payload; the `example/fixture.zip` is a documented test fixture, not a real archive.
- Recommended commit boundary: land this national-source kit as a separate commit after the NorTaxa simplification commit already at HEAD `a135ab9f4c708c2ebdd82ebd02656ef717f3b6c9`. That keeps "simplify acquisition" and "make national sources extensible" as distinct reviewable changes.
