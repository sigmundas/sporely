...
def _migrate_mount_and_stain_settings(cursor: sqlite3.Cursor) -> None:
    ...
    merged_mounts = list(kept_mounts) if kept_mounts else list(_DEFAULT_MOUNT_MEDIA)
    ...
    if not merged_stains:
        merged_stains = list(_DEFAULT_STAIN_TYPES)
...
def init_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS observations (
            ...
            sharing_scope TEXT DEFAULT 'public',
            location_public INTEGER DEFAULT 1,
    ...
    for stmt in (
        ...
        "ALTER TABLE observations ADD COLUMN sync_error_code TEXT",
        "ALTER TABLE observations ADD COLUMN sync_error_message TEXT",
        "ALTER TABLE observations ADD COLUMN sync_blocked_reason TEXT",
        "ALTER TABLE observations ADD COLUMN sync_blocked_at TIMESTAMP",
        "ALTER TABLE observations ADD COLUMN is_draft INTEGER DEFAULT 1",
        "ALTER TABLE observations ADD COLUMN sharing_scope TEXT DEFAULT 'public'",
