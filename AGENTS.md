...
- Do not bypass canonical policy functions: `cloud_image_bytes_desired` for byte-storage decisions, `_reconcile_local_image_cloud_id` for identity repair, the tombstone helpers for deletion intent, and the snapshot store/load helpers for baselines.
