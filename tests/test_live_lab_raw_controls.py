...
def _wait_for_pending_raw_preview(state, timeout_ms: int = 2000) -> None:
    qapp = _qapp()
    deadline = max(1, int(timeout_ms // 10))
    for _ in range(deadline):
        qapp.processEvents()
        captures = list(getattr(state, "_pending_raw_captures", []) or [])
        if captures:
            all_ready = True
            for capture in captures:
                preview_path = getattr(capture, "preview_path", None)
                if preview_path and not Path(str(preview_path)).exists():
                    all_ready = False
                    break
            if not all_ready:
                QTest.qWait(10)
                continue
        worker_thread = getattr(state, "_pending_raw_preview_worker_thread", None)
        active_request = getattr(state, "_pending_raw_preview_active_request", None)
        queue = getattr(state, "_pending_raw_preview_job_queue", None)
        thread_running = bool(getattr(worker_thread, "isRunning", lambda: False)()) if worker_thread is not None else False
        if (not captures or all_ready) and active_request is None and not thread_running and not queue:
            break
        QTest.qWait(10)
    qapp.processEvents()


def _wait_for_preview_file_json(preview_path: Path, timeout_ms: int = 2000) -> None:
    qapp = _qapp()
    deadline = max(1, int(timeout_ms // 10))
    for _ in range(deadline):
        qapp.processEvents()
        try:
            json.loads(Path(preview_path).read_text(encoding="utf-8"))
            return
        except Exception:
            QTest.qWait(10)
    qapp.processEvents()
...
def _build_raw_controls_state() -> SimpleNamespace:
    ...
    state.raw_controls = RawProcessingControls(show_tone_controls_when_disabled=True)
    state.raw_curve_preview_widget = live_lab_tab.RawCurvePreviewWidget()
    state._raw_auto_level_settings_for_source = lambda source, settings: _fake_auto_level_settings_for_source(source, settings)
    ...
    state._raw_curve_preview_source_path = lambda: live_lab_tab.LiveLabTab._raw_curve_preview_source_path(state)
    state._raw_curve_preview_fallback_rgb = lambda: live_lab_tab.LiveLabTab._raw_curve_preview_fallback_rgb()
    state._raw_curve_preview_analysis_from_path = lambda source_path, settings=None: live_lab_tab.LiveLabTab._raw_curve_preview_analysis_from_path(
        state,
        source_path,
        settings,
    )
    ...
    state._raw_preview_proxy_cache_key = lambda source_path, settings: live_lab_tab.LiveLabTab._raw_preview_proxy_cache_key(
        state,
        source_path,
        settings,
    )
    state._raw_preview_proxy_for_source = lambda source_path, settings: _fake_render_raw_preview_proxy_rgb(
        source_path,
        settings=settings,
    )
    state._rgb_to_pixmap = lambda rgb: live_lab_tab.LiveLabTab._rgb_to_pixmap(rgb)
    state._raw_preview_rgb_for_source = lambda source_path, settings: live_lab_tab.LiveLabTab._raw_preview_rgb_for_source(
        state,
        source_path,
        settings,
    )
    state._pending_raw_preview_job_queue = []
    state._pending_raw_preview_active_request = None
    state._pending_raw_preview_worker_thread = None
    state._pending_raw_preview_worker = None
    state._pending_raw_preview_job_counter = 0
    state._pending_raw_preview_latest_job_id = 0
    state._pending_raw_preview_request_for_capture = lambda capture, *, kind="selected": live_lab_tab.LiveLabTab._pending_raw_preview_request_for_capture(
        state,
        capture,
        kind=kind,
    )
    ...
    state._start_pending_raw_preview_job = _start_pending_raw_preview_job
    state._finish_pending_raw_preview_job = lambda: live_lab_tab.LiveLabTab._finish_pending_raw_preview_job(state)
    state._on_pending_raw_preview_job_finished = lambda result: live_lab_tab.LiveLabTab._on_pending_raw_preview_job_finished(state, result)
    state._pending_raw_commit_job_queue = []
    state._pending_raw_commit_active_request = None
    state._pending_raw_commit_worker_thread = None
    state._pending_raw_commit_worker = None
    state._pending_raw_commit_job_counter = 0
    state._pending_raw_commit_batch_mode = None
    state._pending_raw_commit_batch_total = 0
    ...
    state._pending_raw_commit_begin_batch = lambda *, kind, total: live_lab_tab.LiveLabTab._pending_raw_commit_begin_batch(
        state,
        kind=kind,
        total=total,
    )
    state._pending_raw_commit_has_request_for_capture = lambda capture, *, kind=None: live_lab_tab.LiveLabTab._pending_raw_commit_has_request_for_capture(
        state,
        capture,
        kind=kind,
    )
    ...
    state._enqueue_pending_raw_commit_job = lambda request: live_lab_tab.LiveLabTab._enqueue_pending_raw_commit_job(state, request)
    state._start_next_pending_raw_commit_job = lambda: live_lab_tab.LiveLabTab._start_next_pending_raw_commit_job(state)
    def _start_pending_raw_commit_job(request):
        state._pending_raw_commit_active_request = request
        batch_mode = state._pending_raw_commit_batch_mode or getattr(request, "kind", "save_current")
        total = int(state._pending_raw_commit_batch_total or 0)
        if batch_mode == "save_all" and total > 0:
            state._set_pending_raw_commit_busy(
                True,
                state.tr("Saving RAW {current} of {total}: {name}").format(
                    current=min(int(state._pending_raw_commit_batch_completed) + 1, total),
                    total=total,
    ...
    state._start_pending_raw_commit_job = _start_pending_raw_commit_job
    state._on_pending_raw_commit_job_finished = lambda result: live_lab_tab.LiveLabTab._on_pending_raw_commit_job_finished(state, result)
    state._finish_pending_raw_commit_job = lambda: live_lab_tab.LiveLabTab._finish_pending_raw_commit_job(state)
    state._present_raw_preview = lambda **kwargs: live_lab_tab.LiveLabTab._present_raw_preview(state, **kwargs)
    state._schedule_raw_edit_preview_save = lambda *args, **kwargs: live_lab_tab.LiveLabTab._persist_raw_edit_preview(state)
    state._schedule_pending_raw_preview_save = lambda *args, **kwargs: live_lab_tab.LiveLabTab._persist_pending_raw_preview(state)
    state._persist_raw_edit_preview = lambda: live_lab_tab.LiveLabTab._persist_raw_edit_preview(state)
    state._persist_pending_raw_preview = lambda: live_lab_tab.LiveLabTab._persist_pending_raw_preview(state)
    def _fake_save_raw_preview_jpeg(rgb, destination, source_path, source_capture_datetime=None):
        del rgb, source_capture_datetime
        preview_path = Path(destination)
        preview_path.parent.mkdir(parents=True, exist_ok=True)
        settings = None
        edit_session = getattr(state, "_raw_edit_session", None)
        if edit_session is not None and str(getattr(edit_session, "preview_path", "") or "") == str(preview_path):
            settings = getattr(edit_session, "working_settings", None)
        if settings is None:
            for capture in list(getattr(state, "_pending_raw_captures", []) or []):
                if str(getattr(capture, "preview_path", "") or "") == str(preview_path):
                    settings = getattr(capture, "raw_settings", None)
                    break
        if settings is None:
            settings = getattr(state, "_raw_render_settings", RawRenderSettings.default())
        payload = {
    ...
    live_lab_tab.save_raw_preview_jpeg = _fake_save_raw_preview_jpeg
    live_lab_tab.render_raw_preview_proxy_rgb = _fake_render_raw_preview_proxy_rgb
    ...
    state._schedule_raw_edit_preview_refresh = lambda: live_lab_tab.LiveLabTab._refresh_raw_edit_preview(state)
    ...
    state._prune_pending_raw_preview_buffers = lambda keep_capture=None, clear_proxy_cache=False: live_lab_tab.LiveLabTab._prune_pending_raw_preview_buffers(
        state,
        keep_capture,
        clear_proxy_cache=clear_proxy_cache,
    )
    ...
    state._set_pending_raw_preview_busy = lambda *args, **kwargs: None
...
def _fake_render_raw_preview_proxy_rgb(source, *, settings=None, **_kwargs):
    del source, settings, _kwargs
    width = 8
    height = 8
    ramp = np.linspace(0.0, 1.0, width, dtype=np.float32)
    rgb = np.zeros((height, width, 3), dtype=np.float32)
    rgb[..., 0] = ramp
    rgb[..., 1] = np.clip(ramp * 0.9 + 0.05, 0.0, 1.0)
    rgb[..., 2] = np.clip(1.0 - ramp * 0.85, 0.0, 1.0)
    return rgb


def _fake_auto_level_settings_for_source(source, settings):
    del source
...
def test_live_lab_raw_summary_and_slider_state(monkeypatch):
    ...
    assert state.raw_controls.curve_strength_slider.isHidden() is False
...
def test_live_lab_raw_curve_preview_widget_uses_current_preview_histogram(tmp_path, monkeypatch):
    ...
    preview_path = tmp_path / "current_preview.png"
...
def test_live_lab_refresh_selected_pending_raw_preview_enqueues_worker_without_sync_decode(
    ...
    monkeypatch.setattr(
        live_lab_tab.LiveLabTab,
        "_raw_preview_rgb_for_source",
        lambda *args, **kwargs: pytest.fail("pending RAW preview should be queued, not rendered synchronously"),
    )

    live_lab_tab.LiveLabTab._refresh_selected_pending_raw_preview(state)
...
def test_live_lab_pending_raw_preview_result_ignored_when_source_changes(tmp_path, monkeypatch):
    _qapp()
    source_path = tmp_path / "P070020_1.ORF"
    source_path.write_bytes(b"raw-bytes")
    pending = live_lab_tab.PendingRawCapture(
    ...
    state._pending_raw_captures = [pending]
    state._selected_pending_raw_index = 0
    state._pending_raw_preview_active_request = live_lab_tab._PendingRawPreviewJobRequest(
        job_id=1,
        kind="selected",
        capture=pending,
        source_path=source_path,
    ...
    state._pending_raw_preview_latest_job_id = 1

    source_path.write_bytes(b"raw-bytes-updated")
    result = live_lab_tab._PendingRawPreviewJobResult(
        ...
        curve=None,
        preview_path=str(pending.preview_path),
        error=None,
    )
...
def test_live_lab_active_pending_raw_control_change_uses_cached_fast_path_without_busy(
    ...
    pending = live_lab_tab.PendingRawCapture(
        ...
        raw_settings=RawRenderSettings.default(),
        preview_path=tmp_path / "preview.jpg",
        preview_rgb=np.ones((3, 3, 3), dtype=np.float32),
    ...
    state._pending_raw_preview_proxy_cache[
        live_lab_tab.LiveLabTab._raw_preview_proxy_cache_key(state, source_path, pending.raw_settings)
    ] = live_lab_tab._RawPreviewCacheEntry(raw_rgb=np.ones((6, 6, 3), dtype=np.float32))
    ...
    state._enqueue_pending_raw_preview_job = lambda request: enqueue_calls.append(request)

    monkeypatch.setattr(
        live_lab_tab,
        "render_raw_preview_proxy_rgb",
        lambda *args, **kwargs: pytest.fail("cached pending RAW preview should not re-decode"),
    )

    updated_settings = replace(pending.raw_settings, exposure_ev=0.5, light_ev=0.5, dark_ev=0.0)
    assert live_lab_tab.LiveLabTab._apply_raw_settings_to_pending_capture(
        state,
        pending,
        updated_settings,
        render_preview=True,
    ...
    assert pending.preview_rgb is not None
    assert pending.rendered_settings == updated_settings


def test_live_lab_first_missing_pending_raw_preview_enqueues_worker(tmp_path, monkeypatch):
    _qapp()
    state = _build_raw_controls_state()
    source_path = tmp_path / "P070020_1.ORF"
    source_path.write_bytes(b"raw-bytes")
    ...
    state._enqueue_pending_raw_preview_job = lambda request: request_calls.append(request)
    state._raw_preview_rgb_for_source = lambda *args, **kwargs: pytest.fail("missing preview should not decode synchronously")
...
def test_live_lab_commit_all_pending_raw_captures_reports_determinate_progress(tmp_path, monkeypatch):
    ...
    state._update_pending_raw_controls = lambda: None
    state._show_status = lambda *_args, **_kwargs: None
    state._show_pending_raw_capture = lambda *_args, **_kwargs: None
    state._show_session_image = lambda *_args, **_kwargs: None
    state._clear_session_viewer = lambda *_args, **_kwargs: None

    assert live_lab_tab.LiveLabTab._commit_all_pending_raw_captures(state) is True

    assert captured_calls == []
    assert len(started_requests) == 1
    assert started_requests[0].kind == "save_all"
...
def test_live_lab_review_mode_creates_pending_preview_and_defers_db_row(tmp_path, monkeypatch):
    ...
    assert pending.raw_settings.auto_levels is True
    assert state.raw_controls.light_value_label.text() == "0.147"
    assert state.raw_controls.dark_value_label.text() == "0.165"
...
def test_live_lab_pending_raw_controls_still_rerender_after_session_stop(tmp_path, monkeypatch):
    ...
    _wait_for_preview_file_json(preview_path)
    ...
    assert str(preview_path) in state.session_gallery.invalidated_paths
    assert state.viewer_title_label.text().startswith("Pending RAW")
...
def test_live_lab_review_mode_save_current_uses_async_commit_queue(tmp_path, monkeypatch):
    _qapp()
    source_path = tmp_path / "P070020_1.ORF"
    source_path.write_bytes(b"raw-bytes")
    preview_path = tmp_path / "previews" / "P070020_1_preview.jpg"
    ...
    assert live_lab_tab.LiveLabTab._commit_selected_pending_raw_capture(state) is True

    assert captured_calls == []
    assert len(started_requests) == 1
    assert started_requests[0].kind == "save_current"
    assert progress_calls
    assert progress_calls[0][0] is True
    assert progress_calls[0][1] == "Saving RAW: P070020_1.ORF"
    assert progress_calls[0][2] is None
    assert progress_calls[0][3] is None
    assert state._pending_raw_commit_is_busy() is True


def test_live_lab_pending_raw_commit_worker_completion_finalizes_on_main_path_and_removes_capture(tmp_path, monkeypatch):
    _qapp()
    source_path = tmp_path / "P070020_1.ORF"
    source_path.write_bytes(b"raw-bytes")
    preview_path = tmp_path / "previews" / "P070020_1_preview.jpg"
    ...
    request = state._pending_raw_commit_request_for_capture(state._pending_raw_captures[0], kind="save_current")
    assert request is not None
    state._pending_raw_commit_active_request = request
    state._pending_raw_commit_batch_mode = "save_current"
    ...
    assert any("Saved pending RAW capture P070020_1.ORF." in text and tone == "success" for text, tone, _ in statuses)


def test_live_lab_pending_raw_commit_failure_clears_busy_and_reenables_controls(tmp_path, monkeypatch):
    _qapp()
    source_path = tmp_path / "P070020_1.ORF"
    source_path.write_bytes(b"raw-bytes")
    preview_path = tmp_path / "previews" / "P070020_1_preview.jpg"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(RawRenderingUnavailableError("decode failed")),
    )

    assert live_lab_tab.LiveLabTab._commit_selected_pending_raw_capture(state) is True

    assert state._pending_raw_captures[0].status == "failed"
    assert state._pending_raw_commit_is_busy() is False
    assert state.pending_raw_save_btn.isEnabled() is True
...
def test_live_lab_review_mode_save_all_after_session_stop_commits_every_pending_raw(tmp_path, monkeypatch):
    ...
    assert any("Saved 2 RAW captures." in text and tone == "success" for text, tone, _ in statuses)
...
def test_live_lab_review_mode_falls_back_to_companion_jpeg_when_raw_preview_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(live_lab_tab, "render_raw_preview_proxy_rgb", failing_preview)
    state._raw_preview_proxy_for_source = lambda *args, **kwargs: (_ for _ in ()).throw(RawRenderingUnavailableError("rawpy unavailable"))
