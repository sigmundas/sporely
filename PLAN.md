...
# Sporely-Py: Desktop Client Architecture & Migration Tasks

## 1. Context for AI Agent
You are assisting with the migration of `sporely-py`, a PySide6 desktop application. 

**The Architecture Constraints:**
*   **Cloud Storage:** Cloudflare R2 (stores master images as `.avif` or `.jpg`). CDN Domain: `https://cdn.sporely.com`
*   **Local Desktop Cache:** To preserve PySide6 UI speed, **all** cloud images must be cached locally as standard `.jpg` files. Local `pillow-heif` AVIF decoding is strictly prohibited for cloud syncing to prevent UI thread blocking.
*   **The Cloudflare CDN Trick:** When fetching cloud images, `sporely-py` must append Cloudflare's image resizing prefix to force the edge server to perform the AVIF-to-JPEG conversion: `.../cdn-cgi/image/format=jpeg/[FILENAME]`.
*   **Dependencies:** `PySide6`, `Pillow`, `requests`. `pillow-heif` is strictly reserved for local drag-and-drop ingestion of Apple formats.
*   **Existing Codebase:** Do not rewrite the dynamic Qt `QPixmap` spore measurement thumbnail logic; it already works perfectly and will ingest the new local JPEGs natively.

---

## 2. Implementation Tasks

Please execute the following tasks sequentially. Do not move to the next task until the current one is tested and verified.

### Task 1: The Core Sync Pipeline
**Goal:** Establish the cloud-to-local syncing mechanism without blocking the UI.
*   Write a `sync_image_to_local(cloud_filename: str) -> str` function using `requests`.
*   Construct the fetch URL using the CDN trick to guarantee a JPEG response, regardless of the cloud source file extension.
*   Save the resulting byte stream as a `.jpg` in the local cache directory.
*   Ensure the local database properly maps the cloud filename (e.g., `spore_1.avif`) to the newly cached local file (e.g., `spore_1.jpg`).

### Task 2: Asynchronous Field Photo Thumbnails
**Goal:** Prevent PySide6 UI freezing when generating 224x224 gallery thumbnails.
*   Refactor the existing `ImageGalleryWidget.set_observation_id()` flow.
*   Move the existing PIL-based `generate_all_sizes(filepath, image_id)` function call into a background `QThread` or `QRunnable`.
*   Ensure the background worker emits a Qt Signal containing the path to the newly generated thumbnail, so the main thread can safely load the `QPixmap` into the UI.

### Task 3: Verify Dynamic Spore Thumbnails (No Rewrite)
**Goal:** Confirm the existing Qt measurement cropping pipeline works with the new local JPEGs.
*   Review `_get_gallery_thumbnail()` and `create_spore_thumbnail()`.
*   Ensure that they safely accept the file paths generated by Task 1. (No code changes should be necessary here, this is a validation step).

### Task 4: The Local Import Bridge (HEIC/HEIF)
**Goal:** Allow users to drag-and-drop local Apple photos without breaking the JPEG-only local pipeline.
*   Initialize `pillow_heif.register_heif_opener()` at application boot.
*   Create an ingestion function that intercepts local `.heic` or `.heif` files selected by the user.
*   Use Pillow to open the file, strip EXIF orientation (via `ImageOps.exif_transpose`), and save it to the local cache folder as a standard `.jpg`. 
*   Pass this new `.jpg` path into the application's standard processing and cloud-upload queue.
